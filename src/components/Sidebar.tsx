...
export default function Sidebar({ current, onChange, collapsed, onToggle }: Props) {
  ...
  return (
    <aside style={{ ...s.sidebar, width: w }}>
      {/* Logo + toggle */}
      <div style={{ padding: collapsed ? '1rem 0' : '0 1rem', display: 'flex', alignItems: 'center', justifyContent: collapsed ? 'center' : 'space-between', borderBottom: '1px solid #F1F5F9', height: '56px', flexShrink: 0 }}>
        {!collapsed && (
        ...
        <button onClick={onToggle} style={s.toggleBtn} title={collapsed ? 'Expandir menú' : 'Colapsar menú'}>
          {collapsed
      ...
      <nav style={{ flex: 1, padding: '0.75rem 0.5rem', display: 'flex', flexDirection: 'column', gap: '2px', overflowY: 'auto' }}>
        {!collapsed && <div style={{ fontSize: '0.65rem', fontWeight: 700, color: '#CBD5E1', letterSpacing: '0.1em', padding: '0.25rem 0.875rem 0.5rem', textTransform: 'uppercase' }}>Menú</div>}
        {nav.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => onChange(item.id)}
              ...
              <span style={{ color: isActive ? '#2563EB' : '#94A3B8', flexShrink: 0, display: 'flex', transition: 'color 0.15s' }}>
                {item.icon}
              </span>
              {!collapsed && <span style={{ transition: 'opacity 0.2s' }}>{item.label}</span>}
            </button>
          )
        })}
      </nav>
      ...
      {/* User */}
      {!collapsed ? (
        <div style={{ padding: '0.75rem', borderTop: '1px solid #F1F5F9', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '0.6rem 0.75rem', borderRadius: '10px', background: '#F8FAFC', border: '1px solid #F1F5F9', transition: 'background 0.15s' }}>
        ...
        <div style={{ padding: '0.75rem 0', display: 'flex', justifyContent: 'center', borderTop: '1px solid #F1F5F9', flexShrink: 0 }}>
